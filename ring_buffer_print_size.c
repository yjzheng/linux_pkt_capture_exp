...
int main(){
	...
	int snaplen=1500;
	req.tp_frame_size = TPACKET_ALIGN(TPACKET_HDRLEN + ETH_HLEN) + TPACKET_ALIGN(snaplen); //each frame hold one ethernet packet
	req.tp_block_size = sysconf(_SC_PAGESIZE);
	while (req.tp_block_size < req.tp_frame_size) {
		req.tp_block_size <<= 1;
	}
	//ref: http://www.microhowto.info/howto/capture_ethernet_frames_using_an_af_packet_ring_buffer_in_c.html	
	
	req.tp_block_nr = sysconf(_SC_PHYS_PAGES) * sysconf(_SC_PAGESIZE) / (8*req.tp_block_size);
